\chapter{O problema do Carteiro Chinês}
...
Se $G$ é euleriano, então as soluções ótimas para o PCC são os circuitos eulerianos do grafo.
...
\begin{proof}
    ...
    \begin{figure}[H]
        ...
        \begin{tikzpicture}[auto,node distance=3cm, every loop/.style={},thick,main node/.style={circle,draw,font=\sffamily\Large}]
          ...
          \path[every node/.style={font=\sffamily\small}]
              (1) edge node [above] {$x_{14} = 0$} (4)
            (2) edge node [right] {0} (1)
            (3) edge node [above] {0} (2)
            (4) edge node [left] {0} (3)
            ;
        \end{tikzpicture}
    ...
    \begin{figure}[H]
        ...
        \begin{tikzpicture}[auto,node distance=3cm, every loop/.style={},thick,main node/.style={circle,draw,font=\sffamily\Large}]
          ...
          \path[every node/.style={font=\sffamily\small}]
              (5) edge node [above right] {$x_{45} = 1$} (4)
              (1) edge node [above] {0} (4)
            (1) edge node [above] {0} (3)
            (2) edge[red] node [right] {$x_{12} =  1$} (1)
            (3) edge[red] node [above] {$x_{23} = 1$} (2)
            (4) edge node [left] {0} (3)
            ;
        \end{tikzpicture}
    ...
    \begin{figure}[H]
        ...
        \begin{tikzpicture}[auto,node distance=3cm, every loop/.style={},thick,main node/.style={circle,draw,font=\sffamily\Large}]
          ...
          \path[every node/.style={font=\sffamily\small}]
              (5) edge node [above right] {$x'_{45} = 1$} (4)
              (1) edge node [above] {0} (4)
            (1) edge node [above] {0} (3)
            (2) edge[gray, dotted] node [right] {} (1)
            (3) edge[gray, dotted] node [above] {} (2)
            (4) edge node [left] {0} (3)
            ;
        \end{tikzpicture}
...
Segue no algoritmo \ref{pcc-pseudocode} uma versão em pseudocódigo do algoritmo descrito.
    ...
    \begin{algorithm}
        ...
        \begin{algorithmic}[1]
        \Function{pcc}{G}
        \State $(V, E, dist) \gets G$ \Comment{$dist$ representa o custo de cada aresta de $E$ }
        ...
        \Function{expande}{uv, E, dist, distMin} \Comment{ descomprime arestas de $M$ }
        \If{$u = v$}
        \Return $\emptyset$
        \EndIf
        \For{$w : uw \in E$}
            \If{distMin[u][v] = dist[u][w] + distMin[w][v]} 
...
Nos casos de grafos com um número pequeno de vértices ($\leq 24$ vértices), é possível desenvolver uma solução usando programação dinâmica para encontrar tal emparelhamento, esse caso é ilustrado na explicação do problema tratado no apêndice \ref{prb:pcc-bitmask}.

Para ilustrar um uso do PCC que foge um pouco da área de grafos, explica-se no apêndice \ref{prb:pcc} o desafio ``Sereja and the Arrangement of Numbers'', do site Codeforces, que requer uma aplicação interessante do PCC para montar uma sequência de letras com uma característica especial.
    ...
    Uma grande diferença na solução do PCC é que, para digrafos, não vale o lema \ref{lemma-pcc}, que garante a existência de uma solução para todo PCC em que cada aresta do grafo é percorrida no máximo duas vezes.
    ...
    O algoritmo \ref{di-pcc-pseudocode} é a implementação em pseudocódigo da solução descrita acima.
    ...
    As sucessivas duplicações de caminho realizadas em $G$ fazem com que todos os vértices possuam grau de saída e entrada iguais, tornando o novo grafo euleriano (teorema  \ref{euler-digraph}). 

    No quarto passo deriva-se um circuito euleriano para o digrafo construído.


\item[\textbf{4º}]  Um possível circuito euleriano $\mathcal{C}$ que se inicia no vértice $a$ é o seguinte:
    ...
    Define-se como \textbf{grau total} de um vértice $v$, $\delta_t(v)$, a soma do grau de arestas não direcionadas, grau de entrada e saída, ou seja $\grt(v) := \gr(v) + \gre(v) + \grs(v)$.
    ...
    \begin{algorithm}
        ...
        \begin{algorithmic}[1]
        \Function{Misto}{G, c} 
        \State $(V, A, E) \leftarrow G$
            \State $G' \gets $GRAU\_TOTAL\_PAR$(G, c)$ \Comment{Torna $\delta_t$ par}
            \State $M, U \gets $IGUALA\_GRAU\_DIR$(G', c)$ \Comment{Iguala $\delta^+$ e $\delta^-$ no grafo $(V, M, U)$}
            \State $M', U' \gets $GRAU\_PAR$(G', M, U)$ \Comment{$\delta$ par e $\delta^+ = \delta^-$ em $(V, M', U')$}
            \State $G_e \leftarrow (V, U', M')$
    ...
    \begin{algorithm}
        ...
        \Function{grau\_total\_par}{G,c}
            ...
            \State \Return $(V, E + E', A + A')$  \Comment {Devolve grafo com $\grt(v)$ par $\forall v \in V$}
        \EndFunction
        \end{algorithmic}
    \end{algorithm}
    ...
    \begin{algorithm}
        ...
        \begin{algorithmic}[1]
        \Function{iguala\_grau\_dir}{G, c}
        \State \Comment Devolve dois multiconjuntos $M$ e $U$, tal que $M$ contém ao menos uma cópia de cada arco de $A$ e algumas arestas de $E$ orientadas, e $U \subseteq E$ contém apenas arestas não orientadas e não pertencentes a $M$.
            ...
            \EndFor
            \For{$uv \in E$}
        \If{$f(vu) \geq 1$ ou $f(uv) \geq 1$} \Comment{Se $custMin > 0$ é impossível que $f(uv) \geq 1$ e $f(vu) \geq 1$} ao mesmo tempo % TODO: PROVAR
    ...
    \begin{figure}[H]
        ...
        \begin{tikzpicture}[auto,node distance=3cm, every loop/.style={},thick,main node/.style={circle,draw,font=\sffamily\Large}]
            ...
            \path[every node/.style={font=\sffamily\small}]
                ...
                ;
        \end{tikzpicture}
        \caption{O arco $ba$ e a aresta orientada $cd$ duplicados pela função IGUALA\_\-GRAU\_DIR (algoritmo \ref{mixed-iguala-grau-dir})}
    ...
    Por esse motivo é necessária a função GRAU\_PAR, do algoritmo \ref{mixed-grau-par}, que deriva de $M, U$ outros multiconjuntos $M'$ e $U'$ que tornam $\delta_t$ par, mantêm a igualdade de $\delta^+$ e $\delta^-$ e por consequência, tornam $\gr$ par para todo vértice do grafo $(V, M', U')$.
    ...
    \begin{algorithm}
        ...
        \begin{algorithmic}[1]
            \Function{grau\_par}{M, U}
                %\Skate \Comenta{Devolve os multiconjuntos $M'$, $U'$, que mantêm, para todo vértice, a igualdade dos graus $\delta^+$ e $\delta^-$ e torna $\delta^+ + \delta^- + \delta$ par.}
                \State $U' \gets U$
    ...
    Como o exemplo apresentado é transformado em um digrafo euleriano após a execução do algoritmo \ref{mixed-grau-par}, basta tomar o circuito euleriano do mesmo para derivar uma solução do grafo misto original, como mostrado na figura \ref{fig:mixed-ex-sol}.
    ...
    \begin{proof}
        ...
        Pode-se afirmar que não existirá em $G'$ um arco ou aresta $uv$ que foi duplicado mais que uma vez na função \ref{mixed-grau-total-par}, pois se existisse poderia-se remover de $G'$ duas cópias de $uv$ que teríamos outro grafo com mesma paridade total de vértices e custo menor.
        ...
        % TODO: Provar que fator de aproximação é justo, mostrar exemplo que chega próximo a 2 a aproximação.
    \end{proof}

    \section{Problema do carteiro rural}
    ...
    As explicações e heurísticas tratadas a seguir foram baseadas no artigo de Eiselt de 1994 \cite{michel}.
        ...
        O algoritmo que será apresentado é uma $\frac{3}{2}$-aproximação polinomial para o PCR em grafos cuja função de custo de suas arestas respeita a desigualdade triangular.
        ...
        \begin{figure}[H]
            ...
            \begin{tikzpicture}[auto,node distance=3cm, every loop/.style={},thick,main node/.style={circle,draw,font=\sffamily\Large}]
                ...
                \path[every node/.style={font=\sffamily\small}]
                    ...
                    (b) edge[line width=2] node [right] {2} (c)
    ...
    O conjunto $E'$ é definido em duas etapas: primeiramente criam-se novas arestas entre os vértices de $V'$ com custo igual ao custo da menor distância entre os mesmos vértices em $G$, tais arestas são representadas em azul na imagem \ref{grafo-intermediario}.
        ...
        \begin{figure}[H]
            ...
            \begin{tikzpicture}[auto,node distance=3cm, every loop/.style={},thick,main node/.style={circle,draw,font=\sffamily\Large}]
                ...
                \path[every node/.style={font=\sffamily\small}]
                    ...
                    ;
            \end{tikzpicture}
            \caption{Grafo intermediário na transformação de $G$ em $G'$ (após o passo 1.)}
        ...
        No exemplo são retiradas de $E'$ uma cópia das arestas $ab, ac, bf, ce, ef$ pois já existem arestas paralelas a estas de mesmo custo.
        Retiram-se também as arestas: $af$ já que $c_{af} = c_{ab} + c_{bf}$, $be$ já que $c_{be} = c_{bf} + c_{fe}$ e $ae$ já que $c_{ae} = c_{af} + c_{fe}$, sobrando assim apenas a aresta $cf$ de custo $3$.
        ...
        No exemplo tal separação gera dois subgrafos $G_1$ e $G_2$, representadas na figura \ref{r-separacao}, cujos conjuntos de vértices são $V_1 = \{a, b, c\}$ e $V_2 = \{f, e\}$ respectivamente.
        ...
        Segue agora uma descrição da heurística de Frederickson como apresentado por Gendreau e Laporte \cite{michel}.
        ...
        \begin{figure}[H]
            ...
            \end{tikzpicture}
            \caption{Circuito euleriano no grafo induzido}
        ...
        A solução descrita é polinomial, e foi implementada com complexidade $\mathcal{O}(|E||V|^2)$, como comentado em mais detalhes no capítulo \ref{cap:code} de implementação.
        ...
        Nesse caso a heurística é denominada \textbf{heurística da arborescência geradora mínima}.
        ...
        Como no grafo exemplo da figura \ref{pcr-digraph} todos vértices possuem ao menos um arco pertencente a $R$, define-se $V' = V$.
        No caso geral em que $V' \neq V$ é necessário criar novas arestas no grafo para representar os caminhos que passavam pelos vértices apagados, exatamente como foi feito no exemplo do PCR para grafos quando o vértice $d$ foi deletado (vide figuras \ref{grafo-original} e \ref{grafo-intermediario}).

        O conjunto de arcos $A'$ será inicialmente acrescido dos arcos $v_iv_j$ para todo par $v_i, v_j \in V'$, de custo igual ao custo do menor caminho de $v_i$ a $v_j$.
		...
		Isto é, cada conjunto $G_i$ será composto apenas por arcos pertencentes a $R$ e será conexo desconsiderando a orientação dos arcos.

        No exemplo apresentado, $G$ possui três componentes, $G_1$ composta pelos vértices $a, b$, $G_2$ composta por $c, e$ e $G_3$ composta por $d, f$, representadas na figura \ref{condensated-dig}.
	...
	\begin{enumerate}
        \item[\textbf{Passo 1.}] 
            ...
            Condensando o digrafo $G'$ em suas componentes $G_1, G_2, G_3$ temos o grafo representado na figura \ref{condensated-dig}.
                ...
                Além disso, pode-se também substituir qualquer conjunto de arcos paralelos por um único arco de menor custo. 
            ...
            \begin{figure}[H]
                ...
                \caption{Digrafo $G'$ após remoção de arcos do Passo 1}
                \label{chu-liu}
            \end{figure}
            ...
            \begin{itemize}
                ...
                \item Se $u, v \in C$ então $uv$ é condensado na criação do vértice $\mathcal{C}$
            \end{itemize}
            ...
            \caption{Em azul representa-se o conjunto de arcos $T$}
            \label{chu-liu-p}
            \end{figure}
        ...
        \begin{figure}[H]
            ...
            \end{tikzpicture}
            \caption{Digrafo induzido por $R\cup T$, com os arcos de $T$ em azul}
            \label{graphRUT}
        \end{figure}


        \item[\textbf{Passo 2.}]
            Encontrar um multiconjunto $M$ de menor custo composto por arcos de $A'$ que torna o digrafo induzido por $R \cup T$ euleriano, ou seja iguala os graus de entrada e saída de todos vértices no digrafo induzido por $R \cup T \cup M$.
            ...
            Voltando ao exemplo, a solução do problema de transporte consiste na utilização dos caminhos de menor custo de $f$ a $a$ e de $f$ a $d$, escoando uma unidade por cada um desses caminhos. 
            Esta solução supre a oferta de duas unidades do vértice $f$ assim como a demanda de uma unidade dos vértices $d$ e $a$.
        ...
        \item[\textbf{Passo 3.}]
			O digrafo induzido pelo multiconjunto de arcos $R \cup T \cup M$ é, pela escolha de $M$, euleriano. 
        ...
        A partir de tal circuito pode-se encontrar uma solução para o PCR do digrafo original $G$ expandindo os arcos contraídos na construção de $G'$ e removendo as duplicatas de arcos artificiais da criação de um supergrafo euleriano.
        ...
        Após a transformação do circuito euleriano em uma solução do PCR no digrafo original temos o seguinte passeio fechado (figura \ref{solucaoPCR}):
        ...
        \begin{figure}[H]
            ...
            \end{tikzpicture}
            \caption{Circuito que resolve o PCR do grafo $G$. Representam-se em pontilhado os arcos de $G$ não percorridos nesta solução.}
            \label{solucaoPCR}
        \end{figure}
	\end{enumerate}

    Finaliza-se assim a execução da heurística da arborescência geradora mínima, que gera uma solução para o PCR de $G$ com custo 62, um valor um pouco acima do custo da solução ótima 40, como demonstrado abaixo na figura \ref{solucaoOtimaPCR}:
        ...
        \begin{figure}[H]
            ...
            \begin{tikzpicture}[node distance=3cm, every loop/.style={},thick,main node/.style={circle,draw,font=\sffamily\Large}]
                ...
                \path[->] (a) edge[line width=2, bend right, below] node {7} (b);
                \path[->] (b) edge[line width=2, below, bend right, sloped] node {6,8} (a);
                \path[->] (a) edge[left] node {1} (c);
                \path[->] (c) edge[line width=2, above] node {2} (e);
                \path[->] (e) edge[dashed, below] node {} (f);

                \path[->] (e) edge[right] node {3} (d);

                \path[->] (d) edge[line width=2, left] node {4} (f);
                \path[->] (f) edge[pos=0.6, below, sloped] node {5} (b);
                \path[->] (c) edge[dashed, above, left] node {} (b);
                \path[->] (d) edge[dashed, below] node {} (b);

            \end{tikzpicture}
            \caption{Uma solução ótima para o PCR no grafo do exemplo}
            \label{solucaoOtimaPCR}
        \end{figure}
    ...
    \section{Problema do carteiro chinês com vento}
        ...
        Este problema possui várias aplicações, por exemplo, podem existir estradas com uma corrente de vento que o carteiro deve percorrer. 
        A corrente de vento tanto pode ajudar o carteiro se ele anda no mesmo sentido que a corrente, quanto pode atrapalhar se ele anda no sentido contrário do vento.
        ...
        Porém, como provado por Meigu Guan \cite{guan-windy}, se todo circuito de um grafo íngreme $G$ possui o mesmo custo independente do sentido em que é percorrido, então é possível encontrar uma solução para o PCV usando um algoritmo polinomial.
        ...
        Essa transformação reduz o problema a um PCC em um grafo não direcionado, que possui solução exata polinomial, vide seção \ref{sec:pcc}.

        Enuncia-se a seguir o teorema \ref{windy-theorem} que garante que uma resposta ótima do PCC no grafo $(G, c')$ equivale a uma resposta ótima, de mesmo custo, do PCV no grafo íngreme original $(G, c)$.
        ...
        \begin{proof}
            ...
            Finalizando assim a prova da igualdade dos quatro somatórios apresentados.
        ...
        \begin{figure}[H]
            ...
            \end{tikzpicture}
            \caption{Grafo $G$, a função custo das arestas é definida na tabela \ref{tabela:exemplopccv}}
            \label{goriginal}
        \end{figure}
    ...
    \end{table}
        ...
        \begin{proof} 
            ...
            Pode-se dividir $T$ em $k$ circuitos de arcos $C_1, C_2, \dots, C_k$. % TODO: provar que pode dividir T em circuitos
...
%    \section{Problema do Carteiro Hierárquico}
%
%    Nesta variação são definidos $k$ subconjuntos de arestas $\{A_1, A_2, \dots, A_k\}$.
%    Como no problema do carteiro chinês original deve-se encontrar um passeio que passe por todas arestas de um grafo $G$, porém esse passeio só poderá percorrer uma aresta pertencente a um subconjunto $A_j$ se o mesmo já tiver percorrido todas arestas pertencentes aos conjuntos $A_i$ com $1 \leq i < j$.
%
