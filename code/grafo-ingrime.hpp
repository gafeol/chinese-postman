...
struct GrafoIngrime{
    ...
    vector<vector<ArestaIngrime>> adj;

    GrafoIngrime () {}

    GrafoIngrime(int n, vector<tuple<int, int, double, double>> arestas): n(n), m((int)arestas.size()) {
        adj.resize(n);
    ...
    void print(){
        printf("GrafoIngrime com %d nós\nArestas:\n", n);
        for(int a=0;a<n;a++){
            for(ArestaIngrime ar: adj[a]){
                if(a <= ar.prox)
