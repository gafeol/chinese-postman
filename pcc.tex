...
Encontra-se nas referências deste trabalho uma implementação do algoritmo de Edmonds intitulada ``Blossom V'' e publicada por Vladimir Kolmogorov em 2009\cite{kolmogorov}.
    ...
    Segue um pseudo-código para tal solução.
    ...
    \State $dist' \gets $ FLOYD\_WARSHALL$(V, E, dist)$
    \For{$v \in V$}
        \State $b(v) \gets \delta^-(v) - \delta^+(v)$
    \EndFor
    ...
    \Function{problema\_de\_transporte}{F, S, dist, b, cap}
        ...
        \For{$v \in S$}
            ...
            \State $E = E \cup \{vt\}$
        \EndFor
        \State \Return FLUXO\_CUSTO\_MINIMO$(s, t, E, dist, cap)$
    \EndFunction
    \end{algorithmic}
    \end{algorithm}
