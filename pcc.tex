...
\item[\textbf{4º}]  Um possível circuito eulerian $\mathcal{C}$ que se inicia no vértice $a$ é o seguinte:
    ...
    \Function{grau\_dir}{G, c}
        \Comment Devolve dois multiconjuntos, $M$ contendo ao menos uma cópia de cada arco de $A$ e algumas arestas de $E$ orientadas, e $U \subseteq E$ contendo apenas arestas não orientadas e não pertencentes a $M$.
        \State $(V, E, A) \gets G$

        \State Minimizar $z$

        \begin{gather*}
            z = \min \quad \sum_{a \in A}c_ax_a + \sum_{e \in E_1}c_ex_e \\
        \begin{aligned}
            \textup{sujeito a}\quad x_s &\geq 0 \textup{ para } s \in A \cup E_1 \\
            x_e &= \{0, 1\} \textup{ para } e \in E_2\\
            \delta^+(v) - \delta^-(v) =  % TODO: COLOCAR AQUI A SOMA DE GRAUS
        \end{aligned}
\end{gather*}
    \EndFunction
