...
\item[\textbf{4º}]  Um possível circuito eulerian $\mathcal{C}$ que se inicia no vértice $a$ é o seguinte:
    ...
    \subsection{Problema do carteiro chinês com vento}
        ...
        Se existente, o valor da linha $i$ e coluna $j$ indica o custo de percorrer a aresta $ij$ de $i$ para $j$, caso contrário, não existe em $G$ aresta ligando $i$ a $j$.
        ...
        Realizando a transformação da função custo $c_{ij}$ para $c'_{ij} = \frac{c_{ij} + c_{ji}}{2}$ temos a seguinte representação de $(G, c')$:
