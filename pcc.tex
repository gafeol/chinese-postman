...
\item[\textbf{4º}]  Um possível circuito eulerian $\mathcal{C}$ que se inicia no vértice $a$ é o seguinte:
    ...
    \subsection{Problema do carteiro chinês com vento}
        ...
        Porém, como provado por Meigu Guan \cite{guan-windy}, se todo circuito de um grafo $G$ possui o mesmo custo independente do sentido em que é percorrido, então é possível encontrar uma solução para o PCCV usando um algoritmo polinomial.
        ...
        \end{theorem}
        \begin{proof} 
            Seja $T$ uma solução para o PCC em $(G, c')$, $T$ será um passeio fechado em $G$, composto por arcos representando a orientação em que cada aresta de $G$ é percorrida.
            ...
            Provaremos a seguir que para um circuito qualquer $C_i$ vale que:
            \[
                \sum_{ij \in C_i} c_{ij} = \sum_{ij \in C_i} c'_{ij}
            \]

            São dois os casos que devemos analisar:
            \begin{itemize}
                \item Se $C_i$ é um circuito de $G$, então pela propriedade que $G$ respeita, vale que $\sum_{ij \in C_i} c_{ij} = \sum_{ij \in C_i} c'_{ij}$
                \item Do contrário, $C_i$ deverá percorrer uma mesma aresta de $G$ duas vezes, não podendo ser considerada assim um circuito de $G$.

                    Como, por definição $C_i$ é um circuito direcionado, o mesmo não pode possuir arcos duplicados.  % TODO: talvez esteja um pouco confusa essa questao de ser um circuito porem nao ser circuito de $G$
                    Sendo assim, deverá existir uma aresta $uv$ de $G$ que é percorrida em ambos sentidos em $C_i$.

                    Pode-se representar $C_i$ como o seguinte circuito direcionado:
                
                    \[
                        C_i = \{ w_1w_2, \dots, w_iu, uv, vw_{i+1}, \dots, w_jv,  vu, uw_{j+1}, \dots w_kw_1\}
                    \]

                    É possível, a partir de tal representação de $C_i$ construir dois novos circuitos direcionados, possivelmente vazios, que não incluam os arcos $uv$ e $vu$:
                    ...
                    \begin{align*}
                        \sum_{ij \in C_i} c_{ij} &= \sum_{ij \in C_i} c'_{ij} \Leftrightarrow \\
                        c_{uv} + c_{vu} + \sum_{ij \in C'_i} c_{ij} +  \sum_{ij \in C''_i} c_{ij}  &= c'_{uv} + c'_{vu} + \sum_{ij \in C'_i} c'_{ij} + \sum_{ij \in C''_i} c'_{ij} \Leftrightarrow \\
                        \sum_{ij \in C'_i} c_{ij} + \sum_{ij \in C''_i} c_{ij}  &= \sum_{ij \in C'_i} c'_{ij} + \sum_{ij \in C''_i} c'_{ij}\\
                    \end{align*}
            \end{itemize}
