...
Encontra-se nas referências deste trabalho uma implementação do algoritmo de Edmonds intitulada ``Blossom V'' e publicada por Vladimir Kolmogorov em 2009\cite{kolmogorov}.
    ...
    \begin{enumerate}
        \item[\textbf{1º}] Definimos como $F$ o conjunto dos vértices que possuam grau de entrada ($\delta^+$) maior que o grau de saída ($\delta^-$) e $S$ o conjunto dos vértices que possuam grau de saída maior que o grau de entrada.
    ...
    Computamos então o custo do menor caminho entre todos os pares de vértices $u, v$ onde $u \in F$ e $v \in S$.
        ...
        Os vértices de $F$ serão vértices de oferta (com função demanda $b$ negativa), cada vértice $u \in F$ terá que escoar $-b(u)$ unidades de fluxo, já os vértices de $S$ serão vértices de demanda, cada vértice $v \in S$ terá que receber $b(v)$ unidades de fluxo.
        ...
        Deve-se então resolver tal problema de transporte minimizando o custo total e respeitando a função demanda de cada vértice.

        \item[\textbf{3º}] A partir de uma solução de custo mínimo encontrada para o problema de transporte devemos derivar um digrafo euleriano baseado em $G$.

            Uma solução do problema de transporte descrito será uma função $f: E' \rightarrow \mathbb{N}$ que determina qual será a quantidade de fluxo transportada por cada arco de $E'$.

            Cada arco $uv$ criado na modelagem do problema de transporte representa um caminho de mesmo custo no digrafo $G$ entre os vértices $u$ e $v$. Se um arco $uv$ da modelagem é escolhido para transportar uma quantidade $f(uv)$ positiva de fluxo, deve-se criar $f(uv)$ cópias de todos arcos do menor caminho de $u$ a $v$ em $G$, chamaremos esse processo de expansão de $uv$.
            ...
            Ao final das expansões necessárias, todos vértices de $G$ possuirão um mesmo valor de grau de entrada e saída, tornando $G$ euleriano pelo teorema \ref{euler-digraph}.
    ...
    Para se resolver o problema de fluxo de custo mínimo, pode-se utilizar um algoritmo de Simplex para Redes como discutido por Orlin \cite{network-simplex}, de complexidade $\mathcal{O}(VE\log V\log(VC))$, sendo $C$ a capacidade máxima dos arcos da rede, ou ainda, uma variação do algoritmo de Edmonds \cite{min-cost}, de complexidade $\mathcal{O}(V^2E^2)$. 
    ...
    \State $f \gets $ PROBLEMA\_DE\_TRANSPORTE$(F, S, dist, b, cap)$
    ...
    \Function{problema\_de\_transporte}{F, S, custo, b, cap}
...
\item[\textbf{4º}]  Um possível circuito eulerian $\mathcal{C}$ que se inicia no vértice $a$ é o seguinte:
    ...
    % Paper do Frederickson, tem ao menos duas heuristicas pro misto ai: http://delivery.acm.org/10.1145/330000/322150/p538-frederickson.pdf?ip=195.176.178.182&id=322150&acc=ACTIVE%20SERVICE&key=FC66C24E42F07228%2E8FBBFD775A27C843%2E4D4702B0C3E38B35%2E4D4702B0C3E38B35&__acm__=1572510516_82601067be7298d3a7ff02d444c4cc9c
