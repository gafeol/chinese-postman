...
Sendo assim, o grafo $G^*$ resultante é euleriano, e a solução para o PCC do grafo $G$ é o circuito euleriano de $G^*$.
    ...
    Analisaremos agora o caso em que o problema do carteiro chinês é aplicado à grafos que possuam apenas arestas direcionadas, que chamaremos de arcos.
    ...
    Um exemplo contra-exemplo para este pensamento é o seguinte:
    ...
    Na esquerda temos um grafo não direcionado qualquer, e na direita temos um grafo similar, que só difere do primeiro em suas arestas. 
    As mesmas substituidas por arcos de sentidos opostos.
    ...
    Como vértices isolados (sem ligação com arestas) não afetam a solução do problema do carteiro chinês, podemos ignorar os mesmos nos grafos analisados.

    Para que um grafo direcionado conte com solução do problema do carteiro chinês é necessário que o mesmo seja fortemente conexo (isso é, ignorando os vértices isolados como discutido anteriormente).
    Do contrário, não existiria passeio algum no grafo em questão que percorresse todas arestas e voltasse ao vértice inicial.

    Quanto tratamos de grafos direcionados (ou digrafos), é necessário refinar a noção de grau de um vértice:
    Por isso, denominamos \textbf{grau de saída} de um vértice $v$ como o número de arcos que saem de $v$, e \textbf{grau de entrada} como o número de arcos que entram em $v$. 

    Além disso, dizemos que um digrafo é euleriano apenas se, para todo vértice, o grau de entrada é igual ao grau de saída.

    De modo análogo ao caso de grafos não direcionados, a solução do problema do carteiro chinês envolve multiplicar arestas do grafo original até que o mesmo seja euleriano.

    Porém, não vale para os digrafos o lema \ref{lemma-pcc}, que indica que existe uma solução para todo PCC em que cada aresta presente na solução estará multiplicada, no máximo, uma vez.

    Um contra-exemplo disso é o caso ilustrado em \ref{counter-lemma}. 
    ...
    Sendo assim, o grafo euleriano derivado do grafo da figura \ref{counter-lemma} deverá possuir duas cópias adicionais do arco de $2$ a $1$. 
    ...
    \section{Problemas resolvidos}
        ...
        No enunciado do problema define-se uma sequência $s$ como bonita quando existe um inteiro $i$ para cada par de valores distintos $u$, $v$ pertencentes a $s$, tal que $s_i = u, s_{i+1} = v$ ou $s_i = v, s_{i+1} = u$.
        ...
        A recompensa total de uma sequência é igual a soma das recompensas de seus valores distintos, ou seja, a recompensa não depende do número de vezes que cada valor aparece na sequência.
        ...
        A entrada do programa, portanto será dada pelos valores $n, m$ e por $m$ pares $q_i, w_i$, indicando que $q_i$ é um valor que pode ser utilizado e que possui recompensa $w_i$, garante-se que todos valores de $q_i$ são distintos e que $w_i$ é positivo.
        ...
        A partir dessa fórmula fechada, é possível descobrir com uma busca binária, em complexidade $\mathcal{O}(\lg{n})$, qual o maior valor de $k$ tal que $|P_k| \leq n$.
    ...
    Para encontrar tal valor, basta manter uma árvore de busca binária balanceada com as $k$ maiores recompensas dos valores disponíveis, o que custa tempo $\mathcal{O}(m\lg k) = \mathcal{O}(m\lg \sqrt{n})$.
        ...
        Uma solução para este problema pode ser encontrada, em C++, neste \href{https://github.com/gafeol/competitive-programming/blob/master/ojs/cf/367/C.cpp}{link}.
        ...
        Uma alternativa viável para este problema é realizar a construção do emparelhamento perfeito usando um algoritmo de programação dinâmica, ao invés do algoritmo de Edmonds.

        Uma possibilidade é memoizar o custo mínimo de emparelhamento perfeito para todo subconjunto de  vértices do grafo condensado. 
