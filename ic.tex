...
\State $E^* \gets E$
\For{$uv \in M$}
    \State $C \gets$ EXPAND$(uv, E, dist, dist')$
    \State $E^* \gets E^* \cup C$
\EndFor
\State $G^* \gets (V, E^*)$

\Return TRILHA\_EULERIANA$(G^*)$
\EndFunction 

\Function{expand}{uv, E, dist, dist'}
\If{$u = v$}
\Return $\emptyset$
\EndIf
\For{$w : uw \in E$}
    \If{dist'[u][v] = dist[u][w] + dist'[w][v]}
        ...
        \Return $uw$  $\cup$ EXPAND$(wv, E, dist, dist')$
    \EndIf
\EndFor
\EndFunction
