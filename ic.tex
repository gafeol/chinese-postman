...
\begin{corollary} 
    Um digrafo $G$ possui uma trilha euleriana se, e somente se, é fracamente conexo, possui no máximo um vértice $v$ tal que $\delta^-(v) - \delta^+(v) = 1$, um vértice $u$ tal que $\delta^+(u) - \delta^-(u) = 1$ e todos outros vértices $w$ de $G$ possuem $\delta^+(w) = \delta^-(w)$.
\end{corollary}

\begin{proof} 

    \\($\Rightarrow$) Se um digrafo $G$ possui uma trilha euleriana $T$, é garantido que o mesmo é fracamente conexo, já que qualquer par de vértices $u, v$ que possui grau positivo estará presente em $T$. 
    Podemos escrever $T$, portanto de um dos seguintes modos:
    \[ T = \{w_1, \dots, w_i = u, w_{i+1} \dots, w_{j-1}, w_j = v, \dots \}\]

    \[ T = \{w_1, \dots, w_i = v, w_{i+1} \dots, w_{j-1}, w_j = u, \dots \}\]

    De qualquer modo, a partir da subtrilha entre $u$ e $v$ em $T$ pode-se derivar um caminho, provando assim que $G$ será fracamente conexo.


    TODO Continuar Prova
