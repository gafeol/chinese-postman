...
\item[\textbf{4º}]  Um possível circuito eulerian $\mathcal{C}$ que se inicia no vértice $a$ é o seguinte:
    ...
    Assim como a variação do PCC para grafos mistos, o PCR é um problema NP-hard.
    ...
    Para realizar a primeira parte da solução do problema, criar uma estensão euleriana de $G$, serão apresentadas tanto heurísticas quanto discussões das soluções ótimas existentes.
        ...
        Em 1979 Christofides sugeriu um algoritmo aproximado para o problema do caixeiro viajante que possui o melhor fator de aproximação para grafos gerais provada \cite{wiki-christofides}. % https://en.wikipedia.org/wiki/Christofides_algorithm

        Será apresentada uma adaptação de tal algoritmo que pode ser aplicada ao PCR.
        ...
        $A'$ é obtido por adicionar ao conjunto $A$ uma aresta $(v_i, v_j)$ para todo par $v_i, v_j \in V'$ cujo custo é igual ao custo do menor caminho de $v_i$ a $v_j$; e retirar-se de $A'$ duplicatas de arestas paralelas de mesmo custo e todas arestas $(u_i, u_j) \in A \setminus R$ tal que $c_{ij} = c_{ik} + c_{kj}$ para algum $k$.
    ...
    O conjunto $A'$ é definido em duas etapas: primeiramente criam-se novas arestas entre os vértices de $V'$ com custo igual ao custo da menor distância entre os mesmos vértices em $G$, tais arestas são representadas em azul na imagem \ref{grafo-intermediario}.
        ...
        Após a criação das novas arestas são deletadas do grafo todas arestas $uv \notin R$ tal que existe $k$ para que $c_{uv} = c_{uk} + c_{kv}$, assim como as arestas paralelas não pertencentes a $R$ de mesmo custo.
        ...
        O conjunto de arestas $R$ induz uma separação no grafo $G'$ de $n$ componentes conexas $G_1, G_2, \dots, G_n$. 
        Ou seja, os conjuntos $G_i$ são compostos por subgrafos conexos maximais que possuem apenas arestas pertencentes a $R$.

        No grafo $G'$ representado na figura \ref{grafo-transformado} existem duas componentes conexas $G_1$ e $G_2$, compostas pelos conjuntos de vértices $V_1 = \{a, b, c\}$ e $V_2 = \{f, e\}$ respectivamente.

        \begin{enumerate}
        \item[\textbf{Passo 1.}]
                Construir uma árvore geradora mínima $T$ em $G'$ que conecte os subgrafos $G_i$.

                No exemplo, uma árvore de custo mínimo que liga os subgrafos $G_1$ e $G_2$ é composta pelos vértices $c$, $f$ e a aresta entre os mesmos, como é possível visualizar na figura \ref{mst}.
        ...
        \begin{figure}[H]
            ...
            \begin{tikzpicture}[auto,node distance=3cm, every loop/.style={},thick,main node/.style={circle,draw,font=\sffamily\Large}]
                ...
                \path[every node/.style={font=\sffamily\small}]
                    ...
                    ;
            \end{tikzpicture}
            \caption{Árvore geradora mínima que conecta os subgrafos $G1$ e $G2$}
            \label{mst}
        \end{figure}
        ...
        Assim como no caso não direcionado, a solução do PCR se baseia em estender o digrafo $G$ em um digrafo $G'$ euleriano, derivando, após isso, do circuito euleriano de $G'$ uma solução para o PCR de $G$.

        A estensão de $G$ em $G' = (V', A')$ se dá de modo similar àquela do caso anterior:

        O conjunto de vértices possui a mesma definição, $V' = \{u \in V : (u, v) \in R para algum v \in V\}$, com a diferença que agora o par $(u, v)$ é ordenado. 
        ...
        Posteriormente remove-se de $A'$ todo arco que pertence também a $A \setminus R$ e cujo custo $c_{ij}$ é igual a $c_{ik} + c_{kj}$ para algum $k$, removem-se também os arcos paralelos de mesmo valor que pertencem a $A \setminus R$.

        Será apresentada agora uma heurística que resolve esse problema.


        TODO: continuar vendo sobre shortest spanning arborescence 
