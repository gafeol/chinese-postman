...
\begin{proof} 
    ...
    ($\Rightarrow$) Se um digrafo $G$ possui uma trilha euleriana $T$, é garantido que o mesmo é fracamente conexo, já que podemos derivar um caminho entre qualquer par de vértices $u, v$ a partir de $T$, como mostra-se a seguir: 
    ...
    Dividiremos essa prova em dois casos em relação a $T$:
    ...
    ($\Leftarrow$) Seja $G$ um digrafo fracamente conexo, com vértices $u, v$ tal que $\delta^+(u) - \delta^-(u) = 1$, $\delta^-(v) - \delta^+(v) = 1$ e $\delta^+(w) = \delta^-(w)$ para todo $w \in V(G)\setminus \{u, v\}$ , um vértice $u$ tal que  e todos outros vértices $w$ de $G$ possuem 
