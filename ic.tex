...
\section{Directed CPP}
    ...
    \subsection{Problemas resolvidos}
        ...
        \subsubsection{\href{https://codeforces.com/problemset/problem/367/C}{Sereja and the Arrangement of Numbers}}
        Define-se uma sequência de $n$ inteiros $a_1, a_2,\dots, a_n$ como uma sequência ``bonita'' se: para todo par $x, y$ valores distintos que pertençam à sequência existe uma posição $i$ tal que $a_i = x, a_{i+1} = y$ ou $a_i = y, a_{i+1} = x$.

        Sua tarefa é contruir uma sequência bonita que use o maior número de elementos distintos possível, sem exceder um tamanho $n$ e que possua o menor custo possível.

        O custo está relacionado com os valores, na entrada do problema você recebe $m$ valores $q_i, w_i$, o primeiro sendo um valor qualquer, e o segundo sendo o custo de se utilizar esse valor em uma solução.
