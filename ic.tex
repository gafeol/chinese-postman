...
\begin{proof} 
    ...
    Se um digrafo $G$ possui uma trilha euleriana $T$, é garantido que o mesmo é fracamente conexo, já que podemos derivar um caminho entre qualquer par de vértices $u, v$ a partir de $T$, como mostra-se a seguir: 
    ...
    Esse procedimento pode ser utilizado para encontrar um caminho entre quaisquer dois vértices de $T$, mas como $T$ é euleriano e possui todos vértices de $G$, prova-se assim que $G$ é fracamente conexo.

    Provaremos agora que $G$ possui todos os vértices $w$ tal que $\delta^+(w) = \delta^-(w)$ ou que existe um par de vértices $u, v$ tal que $\delta^+(u) - \delta^-(u) = 1$, $\delta^-(v) - \delta^+(v) = 1$ e $\delta^+(w) = \delta^-(w)$ para todo $w \in V(G)\setminus \{u, v\}$.

    Analisaremos dois casos em relação à trilha $T$:
    ...
    \begin{itemize}
        \item Se $T$ é um circuito:
            ...
            Todo vértice $w_i$ possui, em cada uma de suas aparições em $T$, um vértice que o precede e um vértice que o sucede, isso implica que em cada aparição de $w_i$ em $T$, dois arcos ligados a $w_i$ podem ser contabilizados, um entrando em $w_i$ e outro saindo do mesmo. 
            ...
            No fim de $T$, há a última aparição de $v$, que contabiliza uma única aresta entrando em $v$, adicionando uma unidade ao grau de entrada do vértice $\delta^+(v)$.

            Finalmente, nota-se que apesar de $v$ ser um vértice extremo, seus graus de saída e entrada também se igualam.

            Provando-se assim que todos vértices possuem grau de saída e entrada iguais, satisfazendo a implicação do corolário. 
    ...
    % Duas possibilidades: ou provo que G' eh fortemente conexo, ou faz na mao a prova com fracamente conexo mesmo
    ...
    Além disso, como todos os vértices possuem grau de saída e entrada iguais em $G'$, deve valer que $w_1 = w_n$, já que do contrário $\delta^-(w_1) - \delta^+(w_1) = 1$ e $\delta^-(w_n) - \delta^+(w_n)$, e portanto que $T$ é um circuito.
    Deste modo, garante-se a existência de uma trilha $T'$ de $w_j$ a $w_i$:

    \[T' = \{w_j, w_{j+1}, \dots, w_n = w_1, w_2, \dots, w_i\} \]

    A partir da trilha $T'$ é possível derivar um caminho de $w_j$ a $w_i$.
    ...
    Sendo assim, pelo teorema \ref{euler-digraph}, $G'$ é euleriano, e possui um circuito euleriano $C$.

    \[ C = \{w_1, w_2, \dots, w_i = u, w_{i+1} = v, \dots, w_n, w_1 \} \]

    Podemos reescrever $C$ do seguinte modo, tornando o arco de $u$ a $v$ o último arco percorrido em $C$:
