...
O grafo apresentado possui uma grande quantidade de circuitos, por isso encontrar uma partição do mesmo em circuitos $C_1, C_2, \dots, C_k$ não é uma tarefa simples.
...
Como em cada iteração deste algoritmo um arco é deletada, o número máximo de iterações que ele pode realizar é limitado pelo número de arcos de um digrafo, sendo assim o mesmo possui complexidade $\mathcal{O}(|E|)$.
...
\item[\textbf{4º}]  Um possível circuito eulerian $\mathcal{C}$ que se inicia no vértice $a$ é o seguinte:
    ...
    %Adicionar valor da solucao otima
    ...
    Implementação do algoritmo de Hierholzer para grafos não direcionados.\\
    \href{https://github.com/gafeol/chinese-postman/blob/master/euler_hierholzer_undirected.cpp}{github.com/gafeol/chinese-postman/blob/master/euler\_hierholzer\_undirected.cpp}
	\end{thebibliography}
