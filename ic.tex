...
Começamos provando que se um grafo é euleriano então todos seus vértices possuem grau par.
...
Sendo assim, se $G$ é euleriano, então todos seus vértices (extremos ou não) possuem grau par.
...
O grau dos vértices dessas componenentes $G'_i$ deverá ser par, já que, ao retirar todas as arestas de um circuito do grafo $G$, diminuimos o grau de um vértice qualquer $v$ em duas vezes o número de aparições de $v$ no circuito. mantendo assim a paridade dos graus. Além disso, cada componenete conexa de $G'$ possuirá uma quantidade de arestas menor do que $k$.

Portanto, pela hipótese da indução, cada uma dessas componentes deverá possuir um circuito euleriano própria. 
Chamaremos de $T_i$ o circuito euleriano da componente $G'_i$.

Segue, assim, o procedimento para a construção de um circuito euleriano para $G$, que chamaremos de $T$:


Definimos como $\cal{T}$ um conjunto de trilhas eulerianas que inicialmente é igual ao conjunto $\{T_1, T_2, \dots, T_k\}$.
...
Se $e$ for um vértice e fizer parte de alguma trilha euleriana $T_i$ pertencente a $\cal{T}$, podemos representar $T_i$ como:

\[
	T_i = \{eu_2, u_2, u_2u_3, u_3, \dots, u_k, u_ke, e\}
\]

Basta então adicionarmos a trilha $T_i$ como representada na trilha $T$ e remover do conjunto $\cal{T}$ a trilha $T_i$, indicando que ela já foi adicionada à trilha $T$.

Enquanto $e$ ainda pertença a alguma trilha $T_j$ de $\cal{T}$, então repetimos o procedimento acima, adicionando $T_j$ à $T$ e removendo a mesma de $\cal{T}$.


Ao final desse procedimento, o conjunto $\cal{T}$ deverá ser vazio, já que toda trilha $T_i$ possui pelo menos um vértice em $C$. Além disso, toda trilha $T_i$ deverá ter sido adicionada à $T$ uma única vez, já que logo após adicionar uma trilha à $T$ já a removiamos de $\cal{T}$, impedindo que ela fosse repetida na trilha euleriana final.
...
\begin{proof}
    ...
    \begin{enumerate}
        ...
        \item $G$ possui dois vértices de grau ímpar. 
			...
			Sendo assim, podemos aplicar o teorema \ref{euler}, provando a existência de um circuito euleriano $G'$, que chamaremos de $T$.
			Isso implica que a aresta $uv$ deve fazer parte do circuito, e portanto podemos escrever $T$ como:
		
			\[
				T = \{u, v, w_1, w_2, \dots, w_k, u\}
			\]
