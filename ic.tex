...
\item[\textbf{4º}]  Um possível circuito eulerian $\mathcal{C}$ que se inicia no vértice $a$ é o seguinte:
    ...
    \section{Problemas resolvidos}
        ...
        Este problema fornece um conjunto $\mathcal{C}$ de $n$ strings de tamanho 3 e pede para que construa-se uma string $S$ de tamanho $n+2$, a única restrição é que o conjunto das substrings de tamanho 3 da string $S$ deve corresponder a $\mathcal{C}$.

        Um exemplo desse problema é o seguinte:

        \[\mathcal{C} = \{aca, aba, aba, cab, bac\}\]

        Uma string $S$ que resolve o problema para o conjunto $\mathcal{C}$ do exemplo é $abacaba$, já que toda substring de tamanho 3 de $S$ possui uma correspondência em $\mathcal{C}$.

        A solução apresentada para este problema envolve a teoria de caminhos eulerianos apresentada:
        ...
        Segue agora a modelagem do exemplo completa:
        ...
        De modo semelhante, podemos derivar uma string a partir de um passeio no grafo modelado, realizando uma junção das strings representadas pelos vértices do passeio:
        A partir do caminho $\{ba, ac, ca, ab\}$ pode-se derivar a string $bacab$, que possui as substrings $\{bac, aca, cab\}$.


        TRAVEI, NÃO SEI COMO EXPLICAR BEM A SOLUÇÃO
