...
\item[\textbf{4º}]  Um possível circuito eulerian $\mathcal{C}$ que se inicia no vértice $a$ é o seguinte:
    ...
    O conjunto $A'$ é definido em duas etapas: primeiramente criam-se novas arestas entre os vértices de $V'$ com custo igual ao custo da menor distância entre os mesmos vértices em $G$, tais arestas são representadas em azul na imagem \ref{grafo-intermediario}.
        ...
        Para guiar a explicação da heurística toma-se, como exemplo, o grafo $G$, a seguir:
    ...
    \begin{figure}[H]
        ...
        \begin{tikzpicture}[node distance=3cm, every loop/.style={},thick,main node/.style={circle,draw,font=\sffamily\Large}]
            ...
            \path[->] (a) edge[below] node {9} (d);
            \path[->] (a) edge[below] node {4} (c);
            \path[->] (b) edge[line width=2, below] node {5} (a);
            \path[->] (c) edge[bend right, above] node {6} (d);
            \path[->] (c) edge[line width=2, above] node {8} (f);
            \path[->] (d) edge[below] node {3} (e);
            \path[->] (d) edge[bend right, below] node {6} (c);
            \path[->] (e) edge[line width=2, below] node {6} (b);
            \path[->] (e) edge[below] node {5} (g);
            \path[->] (f) edge[below] node {4} (g);
            \path[->] (g) edge[line width=2, above] node {6} (d);
        \end{tikzpicture}
        \caption{Digrafo $G$, o conjunto $R$ corresponde aos arcos em negrito}
        \label{konigsberg-graph}
    \end{figure}

    O digrafo $G$ é composto pelas componentes $G_1$ induzida pelos vértices $c, f$, $G_2$, induzida por $d, g$ e $G_3$ induzida por $a, b, e$.
	...
	\begin{enumerate}
        \item[\textbf{Passo 1.}] 
            ...
            Condensando o digrafo $G$ em suas components $G_1, G_2, G_3$ temos a seguinte situação:
            ...
            \begin{figure}[H]
                ...
                \begin{tikzpicture}[node distance=3cm, every loop/.style={},thick,main node/.style={circle,draw,font=\sffamily\Large}]
                    ...
                    \node[main node, dashed] at (2, 2) (g1) {\parbox{1.5cm}{\centering $G_1$}};
                    \node[main node, dashed] at (-2, 0) (g2) {\parbox{1.5cm}{\centering $G_2$}};
                    \node[main node, dashed] at (2, -2) (g3) {\parbox{1.5cm}{\centering $G_3$}};

                    \path[->] (g3) edge[below] node {9} (g2);
                    \path[->] (g3) edge[below] node {4} (g1);
                    \path[->] (g1) edge[bend right, above] node {6} (g2);
                    \path[->] (g2) edge[bend right, below] node {3} (g3);
                    \path[->] (g2) edge[bend right, above] node {6} (g1);
                    \path[->] (g3) edge[bend right, below] node {5} (g2);
                    \path[->] (g1) edge[above] node {4} (g2);
                \end{tikzpicture}
                \caption{Digrafo $G$, o conjunto $R$ corresponde aos arcos em negrito}
                \label{konigsberg-graph}
            \end{figure}
