    %:python3 import sys; print(sys.version)L:python3 import sys; print(sys.version)L:python3 import sys; print(sys.version)PStartPreview para rodar o pdf com mudancas automaticas
...
\item[\textbf{4º}]  Um possível circuito eulerian $\mathcal{C}$ que se inicia no vértice $a$ é o seguinte:
    ...
    \section{Problemas resolvidos}
        ...
        Este problema fornece um conjunto $n$ strings de tamanho 3 e pede para que construa-se uma string $S$ de tamanho $n+2$. 

        A única restrição é que o conjunto das substrings de tamanho 3 da string $S$ deve corresponder ao conjunto das $n$ strings

        Uma subcadeia ("substring") de uma cadeia de caracteres ("string") $S$ é um conjunto contínuo de caracteres de $S$.

        Por exemplo $foo$, $ob$ e $obar$ são exemplos de subcadeias de $foobar$, porém $fob$ e $br$ não podem ser consideradas subcadeias.

        Neste problema recebem-se $n$ cadeias de três caracteres e pede-se, se possível, para construir uma cadeia de caracteres de tamanho $n+2$ que 
