...
Segue agora uma versão em pseudo-código do algoritmo que soluciona o PCC em grafos não direcionados.
    ...
    \begin{algorithmic}[1]
    \Function{pcc}{G}
    % V' conj dos vertices de grau impar
    ...
    \EndIf
    \For{$w : uw \in E$}
        \If{dist'[u][v] = dist[u][w] + dist'[w][v]} \Comment $uw$ pertence a um caminho mínimo de $u$ a $v$
        \State \Return $uw$  $\cup$ EXPANDE$(wv, E, dist, dist')$
        \EndIf
    \EndFor
    \EndFunction

    %\Function{floyd\_warshall}{V, E, dist}
    %\State $minDist \gets \infty$
    %\For{uv: E}
    %    \State $minDist[u][v] \gets dist(u, v)$
    %    \State $minDist[v][u] \gets dist(u, v)$
    %\EndFor
    %\For{k : V}
    %    \For{u : V}
    %        \For{v : V}
    %            \State $minDist[u][v] \gets MIN(minDist[u][v], minDist[u][k] + minDist[k][v])$
    %        \EndFor
    %    \EndFor
    %\EndFor
    %\Return $minDist$
...
Uma alternativa ao algoritmo de Floyd-Warshall é utilizar o algoritmo de Dijkstra $|V'|$ vezes, calculando a distância mínima de cada vértice de grau ímpar a todos outros vértices do grafo. 
...
%
    ...
    \bibitem{tanya}
    \href{https://codeforces.com/contest/508/problem/D}{Problema D do round \#288 (Div. 2)}
    ...
    \href{https://en.wikipedia.org/wiki/Christofides_algorithm}{Página da wikipedia sobre o algoritmo de Christofides}
    ...
    Christofides, Nicos, et al.  
    \href{https://link.springer.com/chapter/10.1007/BFb0121091}{``An algorithm for the rural postman problem on a directed graph.''}
    Netflow at pisa. Springer, Berlin, Heidelberg, 1986. 155-166.
