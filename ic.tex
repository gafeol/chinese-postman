...
\item[\textbf{4º}]  Um possível circuito eulerian $\mathcal{C}$ que se inicia no vértice $a$ é o seguinte:
    ...
    O conjunto $A'$ é definido em duas etapas: primeiramente criam-se novas arestas entre os vértices de $V'$ com custo igual ao custo da menor distância entre os mesmos vértices em $G$, tais arestas são representadas em azul na imagem \ref{grafo-intermediario}.
        ...
        O conjunto de vértices possui a mesma definição, $V' = \{u \in V : (u, v) \in R \text{ para algum } v \in V\}$, com a diferença que agora o par $(u, v)$ é ordenado. 
	...
	\begin{enumerate}
        ...
        \item[\textbf{Passo 2.}]
			Encontrar um multiconjunto de arcos $M$ de menor custo tal que, se adicionado ao subgrafo $R \cup T$ iguala os graus de entrada e saída de todos vértices do subgrafo.

			Este pode ser modelado como um problema de transporte no grafo $G'$. 
			Se um vértice $v \in V(G')$	possui um grau de entrada maior que seu grau de saída (em relação ao subgrafo $R \cup T$) então o mesmo possuirá uma oferta igual ao valor da diferença de ambos, enquanto que se $v$ possui um grau de saída maior que seu grau de entrada, então ele possuirá uma demanda igual a diferença dos graus.

			A resolução deste problema induz um conjunto $M$ de menor custo possível.
        \item[\textbf{Passo 3.}]
			O digrafo $R \cup T \cup M$ é, pela definição de $M$, euleriano. 
			
			Portanto, pode-se derivar um circuito euleriano de tal grafo, a partir de tal circuito euleriano é possível derivar uma solução do PCR para o grafo original $G$.
	\end{enumerate}
